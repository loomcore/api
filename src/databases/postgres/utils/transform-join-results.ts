...
export function transformJoinResults<T>(
    ...
    if (
        leftJoinOperations.length === 0 &&
        innerJoinOperations.length === 0 &&
    ...
    const allJoinAliases = [
        ...leftJoinOperations.map(j => j.as),
        ...innerJoinOperations.map(j => j.as),
    ...
    return rows.map(row => {
        ...
        for (const key of Object.keys(row)) {
            const hasJoinPrefix = [...leftJoinOperations, ...innerJoinOperations].some(join => key.startsWith(`${join.as}__`));
        ...
        for (const operation of operations) {
            ...
            if (operation instanceof LeftJoin || operation instanceof InnerJoin) {
                ...
                if (operation.localField.includes('.')) {
                    ...
                    const relatedJoin = [...leftJoinOperations, ...innerJoinOperations].find(j => j.as === tableAlias);
