...
import {
	runInitialSchemaMigrations,
	runTestSchemaMigrations,
} from "../databases/postgres/migrations/__tests__/test-migration-helper.js";
...
export class TestPostgresDatabase implements ITestDatabase {
	...
	private async createIndexes(connection: PostgresConnection) {
      ...
      `);
			...
			console.log(
				"Note: Indexes may be created later when tables are initialized:",
				error.message,
			);
		}
	}
    ...
    `);
		...
		if (USE_REAL_POSTGRES) {
			await this.postgresClient.query(
				`TRUNCATE TABLE ${result.rows.map((row) => `"${row.table_name}"`).join(", ")} RESTART IDENTITY CASCADE`,
			);
		} else {
			// pg-mem does not support truncating mutliple tables at once yet, so we need to truncate each table individually
			result.rows.forEach(async (row) => {
				await this.postgresClient?.query(
					`TRUNCATE TABLE "${row.table_name}" RESTART IDENTITY CASCADE`,
				);
			});
		}
	}
