...
export class AuthService extends MultiTenantApiService<IUser> {
	...
	constructor(database: IDatabase) {
		...
		this.refreshTokenService = new MultiTenantApiService<IRefreshToken>(
			database,
			"refresh_tokens",
			"refresh_token",
			refreshTokenModelSpec,
	...
	async logUserIn(userContext: IUserContext, deviceId: string) {
		...
		const refreshTokenObject = await this.createNewRefreshToken(
			userContext.user._id,
			deviceId,
			userContext.organization?._id,
		);
		const accessTokenExpiresOn = this.getExpiresOnFromSeconds(
			this.authConfig.jwtExpirationInSeconds,
		);
		...
		if (userContext.user._id === "system") {
			if (
				...
				throw new BadRequestError(
					"User is not authorized to create a user in this organization",
				);
			}
	...
	async changePassword(
		...
		const updatedUsers = await super.update(
			userContext,
			queryObject,
	...
	async getActiveRefreshToken(refreshToken: string, deviceId: string) {
		const refreshTokenResult = await this.refreshTokenService.findOne(
			getSystemUserContext(),
			{ filters: { token: { eq: refreshToken }, deviceId: { eq: deviceId } } },
		);
	...
	async createNewRefreshToken(
		...
		const expiresOn = this.getExpiresOnFromDays(
			this.authConfig.refreshTokenExpirationInDays,
		);
	...
	async sendResetPasswordEmail(emailAddress: string, clientBaseUrl: string) {
		...
		const expiresOn = this.getExpiresOnFromMinutes(
			this.authConfig.passwordResetTokenExpirationInMinutes,
		);
	...
	async resetPassword(
		...
		const retrievedPasswordResetToken =
			await this.passwordResetTokenService.getByEmail(lowerCaseEmail);
		...
		await this.passwordResetTokenService.deleteById(
			getSystemUserContext(),
			retrievedPasswordResetToken._id.toString(),
		);
	...
	getAndSetDeviceIdCookie(req: Request, res: Response) {
		...
		if (isNewDeviceId) {
			...
			const cookieOptions: any = {
				maxAge:
					this.authConfig.deviceIdCookieMaxAgeInDays * 24 * 60 * 60 * 1000,
				httpOnly: true,
			};
