...
export function setBaseApiConfig(theConfig: IBaseApiConfig) {
	if (!isConfigSet) {
		...
		console.warn(
			"BaseApiConfig data has already been set. Ignoring subsequent calls to setBaseApiConfig.",
		);
	}
}
...
export async function initSystemUserContext(database: IDatabase) {
	if (!isConfigSet) {
		throw new Error(
			"BaseApiConfig has not been set. Call setBaseApiConfig first.",
		);
	}
	...
	if (!isSystemUserContextSet) {
		...
		if (config.app.isMultiTenant) {
			...
			if (!metaOrg) {
				throw new Error(
					"Meta organization not found. Please create an organization with isMetaOrg=true before starting the API.",
				);
			}
		}
		...
		console.warn(
			"SystemUserContext has already been set. Ignoring subsequent calls to initSystemUserContext.",
		);
	}
}
